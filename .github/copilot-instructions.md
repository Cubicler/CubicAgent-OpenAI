...
- **Built on CubicAgentKit 2.0.1** - Uses CubicAgent, AxiosAgentClient, and ExpressAgentServer with lazy initialization
...
CubicAgent-OpenAI is built on top of CubicAgentKit 2.0.1's composition architecture with lazy initialization:

**Foundation Dependency:**
```bash
npm install @cubicler/cubicagentkit@^2.0.1
```
...
CubicAgentKit 2.0.1 provides built-in lazy initialization - the agent only connects to Cubicler when the first dispatch request arrives:
...
• **Maintain CubicAgentKit 2.0.1 patterns** - Use CubicAgent, AxiosAgentClient, ExpressAgentServer with lazy initialization
...
• **Handle lazy initialization** - Let CubicAgentKit 2.0.1 handle connection timing automatically
...
• Do not add manual initialization code - let CubicAgentKit 2.0.1 handle lazy initialization

When working on CubicAgent-OpenAI, focus on creating a robust, production-ready OpenAI agent that seamlessly integrates with Cubicler using the CubicAgentKit 2.0.1 foundation while maintaining simplicity and reliability.